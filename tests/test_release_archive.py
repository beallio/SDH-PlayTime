...
class ReleaseArchiveTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        for name in ("package.json", "plugin.json"):
            (self.source / name).write_text(json.dumps({"name": "PlayTime", "version": "old"}))
        ...
        (self.source / "py_modules" / "__pycache__" / "runtime.pyc").write_bytes(b"cache")
    ...
    def test_stable_archive_has_canonical_payload_checksum_and_root(self) -> None:
        ...
        checksum = archive.with_name(f"{archive.name}.sha256")
        self.assertEqual(hashlib.sha256(archive.read_bytes()).hexdigest(), checksum.read_text().split()[0])
        ...
        with zipfile.ZipFile(archive) as zip_file:
            ...
            for path in ("LICENSE", "main.py", "package.json", "plugin.json", "README.md", "dist/", "py_modules/"):
            ...
            for manifest_name in ("package.json", "plugin.json"):
                ...
                self.assertEqual({"name": manifest["name"], "version": manifest["version"]}, {"name": "PlayTime", "version": version})

    def test_custom_nightly_version_does_not_modify_checkout(self) -> None:
        archive = self._build("3.3.0+beallio.gabcdef0")
        self.assertEqual(json.loads((self.source / "package.json").read_text())["version"], "old")
        self.assertEqual(json.loads((self.source / "plugin.json").read_text())["version"], "old")
    ...
    def test_validation_rejects_tampered_checksum_and_unsafe_members(self) -> None:
        ...
        for member_name in (
            ...
            with self.subTest(member_name=member_name), self.assertRaisesRegex(
    ...
    def test_validation_rejects_symlink_directory_member(self) -> None:
        ...
        with self.assertRaisesRegex(release_archive.ArchiveValidationError, "symbolic link"):
            release_archive.validate_archive(archive, "3.3.0+beallio.1")
    ...
    def test_validation_rejects_duplicate_paths(self) -> None:
        ...
        checksum.write_text(f"{hashlib.sha256(duplicate.read_bytes()).hexdigest()}  {duplicate.name}\n")
        with self.assertRaises(release_archive.ArchiveValidationError):
